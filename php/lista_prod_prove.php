<?php
	...
	$campos="producto.producto_id,producto.producto_codigo,producto.producto_farmace,producto.producto_nombre,producto.producto_precio,producto.producto_stock,producto.producto_foto,producto.fecha_venc,producto.producto_ingreso,producto.categoria_id,producto.usuario_id,usuario.usuario_id,usuario.usuario_nombre,usuario.usuario_apellido, proveedor.proveedor_id, proveedor.proveedor_nombre";

	if(isset($busqueda) && $busqueda!=""){

		$consulta_datos="SELECT $campos FROM producto INNER JOIN usuario ON producto.usuario_id=usuario.usuario_id INNER JOIN proveedor ON producto.proveedor_id=proveedor.proveedor_id WHERE producto.producto_codigo LIKE '%$busqueda%' OR producto.producto_nombre LIKE '%$busqueda%' ORDER BY producto.producto_nombre ASC LIMIT $inicio,$registros";
		...
		$consulta_datos="SELECT $campos FROM producto INNER JOIN usuario ON producto.usuario_id=usuario.usuario_id INNER JOIN proveedor ON producto.proveedor_id=proveedor.proveedor_id WHERE producto.proveedor_id='$proveedor_id' ORDER BY producto.producto_nombre ASC LIMIT $inicio,$registros";

		$consulta_total="SELECT COUNT(producto_id) FROM producto WHERE proveedor_id='$proveedor_id'";
	}else{
			        ...
			        <div class="media-content">
			            <div class="content">
			              <p>
			                ...
			                <strong>LOTE:</strong> '.$rows['producto_codigo'].', <strong>FARMACÉUTICA:</strong> '.$rows['producto_farmace'].', <strong>PRECIO:</strong> Bs.'.$rows['producto_precio'].', <strong>STOCK:</strong> '.$rows['producto_stock'].', <strong>PROVEEDOR:</strong> '.$rows['proveedor_nombre'].'<br> <strong>F_INGRESO:</strong> '.$rows['producto_ingreso'].', <strong>F_VENCIMIENTO:</strong> '.$rows['fecha_venc'].', <strong>REGISTRADO POR:</strong> '.$rows['usuario_nombre'].' '.$rows['usuario_apellido'].'
			              </p>
			            </div>
